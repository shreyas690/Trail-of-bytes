...
const GamePage = () => {
  ...
  const [level1Locked, setLevel1Locked] = useState(false);
  const [level1Winner, setLevel1Winner] = useState(null);
  ...
  useEffect(() => {
    ...
    const handleLevel1Locked = (data) => {
      setLevel1Locked(true);
      setLevel1Winner(data.winningTeamName);
      setMessage({
        text: `Level 1 completed by ${data.winningTeamName}! Redirecting to Level 2...`,
        type: "info"
      });
    ...
    socket.on("level1:locked", handleLevel1Locked);
    ...
    return () => {
      ...
      socket.off("level1:locked", handleLevel1Locked);
    };
            ...
            {level1Locked && (
              <div className="mb-4 p-4 bg-yellow-500/20 border border-yellow-500 rounded-lg">
                <p className="text-yellow-300 font-semibold text-center">
                  🏆 Level 1 completed by {level1Winner}! Please proceed to Level 2.
                </p>
              </div>
            )}
            <Grid cells={gridCells} onClick={handleCellClick} disabled={attemptsLeft <= 0 || level1Locked} />
          </section>
